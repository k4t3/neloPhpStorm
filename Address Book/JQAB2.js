...
function valInput(validate){
	var result = '';
		if($(validate).val() == ''){
			result += 'Fehler\n';
			validate.style.borderColor="#ff0000";
		}
		if($(validate).attr("name") == "number" || $(validate).attr("name") == "mobile"){
			result += valNumber($(validate).val());
		}
		if($(validate).attr("name") == "email"){
			if($(validate).val() !=''){
				result += valEmail($(validate).val());
			}
		}
		if(result == '' && result == 0 ){
			validate.style.borderColor="#dcdcdc";
			tempValues[$(validate).attr('name')] = $(validate).val();
		} else {
			alert(result);
		}
	return result;
